...
void setup() {
  ...
  client.publish("demo","Mensaje Recibido");
}
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if ((char)payload[0] == '1') {
    ...
    client.publish("Accesa/test","encendido");
  }
   else if ((char)payload[0] == '0') {
    digitalWrite(BUILTIN_LED, LOW);  // Turn the LED off by making the voltage HIGH
    client.publish("Accesa/test","Apagado");
  }
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect("ESPClient")) {
      ...
      client.publish("Accesa/test", "Enviando el primer mensaje");
      // ... and resubscribe
      client.subscribe("Accesa/test");
    } else {
...
void loop() {
  ...
  if (now - lastMsg > 3000) {
    ...
    Serial.println(msg);
    client.publish("Accesa/test", msg);
  }
